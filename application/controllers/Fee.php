...
class Fee extends CI_Controller
{
  ...
  public function manage_feetypes()
  {
    if (_is_user_login($this)) {
      ...
      $data["fee_type"] = $this->fee_model->get_fee_type($school_id);
      if ($_POST) {
        ...
        // $this->form_validation->set_rules('month_session', 'Month Session', 'trim|required');
        ...
        if ($this->form_validation->run() == FALSE) {
          ...
          $month_session = $this->input->post("month_session");
          ...
          $this->session->set_flashdata("message", '<div class="alert alert-success alert-dismissible" role="alert">
            <button type="button" class="close" data-dismiss="alert"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
            <strong>Success!</strong> Fee types added successfully
            </div>');
  ...
  public function add_student_fees()
  {
    if (_is_user_login($this)) {
      ...
      $data["fee_type"] = $this->fee_model->get_fee_type($school_id);
      $this->load->model("fee_model");
      $data["student_list"] = $this->fee_model->get_student_fees($school_id);
  ...
  public function edit_student_fees($student_fees_id)
  {
    if (_is_user_login($this)) {
      ...
      $students = $this->fee_model->get_student_by_standard($student_fees_row->standard_id, $school_id);
      $fees_type = $this->fee_model->get_fees_type_by_standard($student_fees_row->standard_id, $school_id);
      ...
      if ($_POST) {
        ...
        if ($this->form_validation->run() == FALSE) {
          ...
          $this->common_model->data_update(
            "student_fees",
            $update_array,
            array("student_fees_id" => $student_fees_id)
          );
          $this->session->set_flashdata("message", '<div class="alert alert-success alert-dismissible" role="alert">
            <button type="button" class="close" data-dismiss="alert"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
            <strong>Success!</strong> Event Update Successfully
  ...
  public function list_student_fees_by_student($student_id)
  {
    if (_is_user_login($this)) {
      ...
      // $data = array();
      $this->load->model("fee_model");
      $data["student_fees"] = $this->fee_model->get_student_fees_list($student_id, $school_id);

      $this->load->view("fee/view_student_fees", $data);
    }
  }

  public function print_student_fees_by_student($student_id)
  {
    if (_is_user_login($this)) {
      $this->load->model('school_model');
      $this->load->model("teacher_model");
      ...
      $this->load->view("fee/fee_print", $data);
    }
  }
  ...
  public function free_type_json()
  {
    ...
    $this->load->model("fee_model");
    $result = $this->fee_model->get_fees_type_by_standard($this->input->post("fee_types"), $school_id);
    echo json_encode($result);
  }
  ...
  public function free_amount_json()
  {
    ...
    $this->load->model("fee_model");
    $result = $this->fee_model->get_fees_remaining_amount($this->input->post("fee_amount"), $this->input->post("student_id"), $school_id);
    echo json_encode($result);
  }
}