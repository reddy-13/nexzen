...
function profile(){
    if(_is_user_login($this)){
      ...
      if(isset($_REQUEST["saveprofile"])){
        ...
        $this->form_validation->set_rules('punch_machine_id', 'School Machine ID', 'required');
        ...
        $this->form_validation->set_rules('school_facebook', 'School Facebook Page Link', 'trim');
        if($this->form_validation->run() == FALSE) 
        {
     ...
     }else
     {
        ...
        $school_profile = array(
            ...
            "punch_machine_id"=>$this->input->post("punch_machine_id"),
