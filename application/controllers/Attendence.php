...
$objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('B1', 'standard')
            ->setCellValue('C1', 'student name')
            ->setCellValue('D1', 'student roll no')
            ->setCellValue('E1', 'present')
            ->setCellValue('F1', 'absent')
            ->setCellValue('G1', 'attendence_reason');
                  $this->load->model('school_model');
                  $this->load->model("teacher_model");
                  $school_data = $this->school_model->get_school_profile();
                  /* get school standard */
                 ...
                 if(_get_current_user_type_id($this) == 1){
                  ...
                  $school_id = $school_data->school_id;
                   ...
                   $student_data = $this->student_model->get_school_standard_student_add_attendence($standard, $school_id);
            ...
            foreach($stud_item as $item){
              ...
              if($item->standard_id == $student->standard_id && $item->student_id == $student->student_id && $item->school_id == $student->school_id){

                 $array = array('attended'=>$item->attended , 'attendence_reason'=>$item->attendence_reason);
                  
                }

                $objPHPExcel->setActiveSheetIndex(0)->setCellValue('A'.$row_index, $student->student_id)
                ->setCellValue('B'.$row_index, $student->standard_title)
                ->setCellValue('C'.$row_index, $student->student_name)
                ->setCellValue('D'.$row_index, $student->student_roll_no)
                ->setCellValue('E'.$row_index, $array['attended'] == 1  ? "Present" : "X" )
                ->setCellValue('F'.$row_index, $array['attended'] == 0  ? "Absent" : "")
                
                
                ->setCellValue('G'.$row_index,$array['attendence_reason']);
...
// Rename worksheet
$objPHPExcel->getActiveSheet()->setTitle('Attendence Data '.date("Y-m-d", strtotime($date)));
                ...
                if ($this->form_validation->run() == FALSE) 
        		{
                 ...
                 if(_get_current_user_type_id($this) == 1){
                  ...
                  $school_id = $school_data->school_id;
                    ...
                    $data["student"] = $this->student_model->get_school_standard_student_add_attendence($_REQUEST['standard'], $school_id );
