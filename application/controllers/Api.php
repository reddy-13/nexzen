     ...
     //$_POST = $_REQUEST;
        ...
        if ($this->form_validation->run() == FALSE) 
  		...
  		{        
                   ...
                   if (!empty($student))
                   {
                       $q2 = $this->db->query("SELECT * FROM `school_detail` WHERE `school_id` = '".$student->school_id."'");
                       $school_data = $q2->row();
    ...
    public function get_student_profile(){
            ...
            if($_REQUEST["student_id"]!=""){
                ...
                //$q = $this->db->query("select * from `student_detail` INNER JOIN categories ON job.category_id = categories.id where job.user_id = ".$_REQUEST["user_id"]);
                 ...
                 INNER JOIN school_detail ON school_detail.school_id = student_detail.school_id 
...
/* get top 10 student by standard */
    ...
    public function timetable(){
     ...
     $data = array(); 
            if(isset($_REQUEST["standard_id"]) && $_REQUEST["standard_id"]!="" && isset($_REQUEST["school_id"]) && isset($_REQUEST["day_id"])){ 
    ...
    public function get_book_by_standard(){        
     ...
     public function list_student_fees_by_student(){        
        $data = array(); 
           if($_REQUEST["student_id"]!=""){  
                        $this->db->where("student_id",$_REQUEST["student_id"]);
                        $q = $this->db->get("student_detail");
                        $student = $q->row();
                        if(empty($student)){
                            $data["responce"] = false;  
                        ...
                        student_detail.student_address,
                        student_detail.student_city,
                        student_detail.student_roll_no,
                        student_detail.student_branch,
