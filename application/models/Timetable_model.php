...
class Timetable_model extends CI_Model{
     ...
     public function get_day_by_id($school_id){
        $q = $this->db->query("select timetable.*,standard.standard_title,teacher_detail.teacher_name,days.day_name from timetable
                                inner join standard on standard.standard_id = timetable.standard_id
                                 ...
                                 inner join days on days.id = timetable.day_id where timetable.school_id=".$school_id);
       return $q->result();
    } 
    ...
    public function get_teacher($school_id){
        $q = $this->db->query("select * from teacher_detail where school_id=".$school_id);
       return $q->result();
    } 
}
?>